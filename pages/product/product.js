...
Page({
  ...
  data: {
    ...
    tip_table: ['备注说明', '两室一厅的窗帘约为20㎡。', '用于别墅、会议室等场所大的窗帘，另行计算。', '包含上门拆卸、安装服务。'],
    guar: [
  ...
  footer_num: function (goods) {//goods商品数据
    ...
    var parsePrice = parseFloat(total_price).toFixed(3);
    var subPrice = parsePrice.substring(0, parsePrice.toString().length - 1);
    that.setData({
      total_num: total_num,
      total_price: subPrice,
      dif_price: (40 - subPrice).toFixed(2),
    })
  },
